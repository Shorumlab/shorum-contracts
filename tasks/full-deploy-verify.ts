...
const LENS_HUB_NFT_NAME = 'Allo Protocol Profiles';
const LENS_HUB_NFT_SYMBOL = 'APP';
...
task(
  'full-deploy-verify',
  'deploys the entire Allo Protocol with explorer verification'
).setAction(async ({}, hre) => {
  ...
  console.log('\n\t-- Deploying backerFeeFollowModule --');
  ...
  await waitForTx(
    lensHub.whitelistFollowModule(backerFeeFollowModule.address, true, { nonce: governanceNonce++ })
  );
  // await waitForTx(
